...
const TaskCard = ({ task }) => {
  ...
  return (
    <>
      <div className='w-full h-fit bg-black/50 shadow-md p-4 rounded-xl '>
        <div className='w-full flex justify-between'>
          ...
          >
            ...
            <span className='uppercase'>أولويه {PIORIRTYAR[task?.priority]} </span>
          </div>
        ...
        <>
          <div className='flex items-center gap-2 text-white '>
            ...
            />
            <h4 className='line-clamp-3 text-white'>{task?.title}</h4>
          </div>
        ...
        <div className='flex items-center justify-between mb-2'>
          <div className='flex items-center gap-4'>
            <div className='flex gap-1 items-center text-sm text-gray-500'>
              <span className="text-gray-300">{task?.activities?.length}</span>
              <BiMessageAltDetail />
            </div>
            <div className='flex gap-1 items-center text-sm text-gray-500 '>
      ...
      <AddSubTask open={open} setOpen={setOpen} id={task._id} url='task'/>
    </>
  );
};
